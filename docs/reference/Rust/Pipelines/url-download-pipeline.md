...
`UrlDownloadPipeline` is an abstract class designed to handle uploads using URL-based storage adapters (`UrlStorageAdapter`).
...
| ----------------------------------------------------------------------------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `execute(local_dir, urls, strategy,  plan, options)`                                      | `Result<(), DownloadError>`                     | Performs a full download process for a directory using urls.                                           |
...
| `download_all_missing_chunks(download_urls, target, options)`                             | `Result<Arc<dyn ChunkSource>, DownloadError>`   | This method will download first all needed chunks via urls, and save them temporary on disk or memory. |
...
| `urls.download_urls` | `HashMap<String, ChunkUrlInfo>` | The urls identified by hash to download chunks. See ChunkUrlInfo below.                                           |
| `urls.index_url`     | `String`                        | Url to download remote rd-index.json.                                                                             |
...
| `plan`               | `Option<DeltaPlan>`             | Optional DeltaPlan for reference, if none provided will try to generate one.                                      |
...
| Name           | Type                            | Description                                                             |
| -------------- | ------------------------------- | ----------------------------------------------------------------------- |
| `downloadUrls` | `HashMap<String, ChunkUrlInfo>` | The urls identified by hash to download chunks. See ChunkUrlInfo above. |
...
| `chunk_source` | `Arc<dyn ChunkSource>`    | `ChunkSource` to download corrupt chunks in case of invalid files. See [ChunkSource](/docs/reference/Rust/chunk-sources/chunk-source) |
...
| `phase`      | `DownloadPhase`            | Current download phase (downloading, reconstructing or deleting). |
